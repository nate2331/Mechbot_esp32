...
Identity: `MAKER_SPI_V3C_SINGLE_TX`. This isolated sketch is for IMU diagnosis;
...
This version sends a whole outgoing packet in one SPI call, then clocks any
longer RX tail under the same CS assertion. Buffers are explicitly word-aligned.
Reads retain the four-byte header followed by payload. It captures the first
