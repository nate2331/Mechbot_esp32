...
def main():
    ...
    import serial
    home = Path('/home/nate')
    directory = Path(tempfile.mkdtemp(prefix='maker-powered-encoder-', dir=home))
    report = {'scope':'user-supervised raised-wheel encoder check; IMU not required',
    ...
    def save():
        ...
        (home/'maker-powered-encoder-latest.json').write_text(json.dumps(report,indent=2))
    ...
    try:
        ...
        for name, values, direction in [('RL forward',[0,0,177,0],1),('RL reverse',[0,0,177,0],-1),
                                         ('All forward',[177]*4,1),('All reverse',[177]*4,-1)]:
