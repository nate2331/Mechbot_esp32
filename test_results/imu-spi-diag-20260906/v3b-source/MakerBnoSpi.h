...
class MakerBnoSpi : public Adafruit_BNO08x {
 ...
 private:
  ...
  void exchange(const uint8_t* tx, unsigned txLen, uint32_t receivedUs) {
    ...
    spi_->endTransaction();
    if (txLen >= 4 && txScratch_[2] == 2 &&
        ((static_cast<unsigned>(txScratch_[0]) |
          (static_cast<unsigned>(txScratch_[1]) << 8)) & 0x7FFFU) == txLen) {
      nextControlSequence_ = static_cast<uint8_t>(txScratch_[3] + 1U);
    }
