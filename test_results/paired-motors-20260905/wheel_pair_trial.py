...
def main():
    ...
    import serial
    home = Path('/home/nate')
    directory = Path(tempfile.mkdtemp(prefix='maker-paired-motors-', dir=home))
    report = {'scope':'user-requested raised-wheel pair comparison; 20 V drill battery through regulator set to 9 V; trusted encoder calibration',
    ...
    def save():
        ...
        (home/'maker-paired-motors-latest.json').write_text(json.dumps(report,indent=2))
    ...
    try:
        ...
        for name, values, direction in [
                ('Front forward',[177,177,0,0],1), ('Front reverse',[177,177,0,0],-1),
                ('Rear forward',[0,0,177,177],1), ('Rear reverse',[0,0,177,177],-1),
                ('Left forward',[177,0,177,0],1), ('Left reverse',[177,0,177,0],-1),
                ('Right forward',[0,177,0,177],1), ('Right reverse',[0,177,0,177],-1)][int(sys.argv[1]):int(sys.argv[2])]:
