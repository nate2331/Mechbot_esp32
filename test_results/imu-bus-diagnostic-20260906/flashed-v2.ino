...
bool readPacket() {
  ...
  if (actual < 4 || actual > n || data[2] > 5 || data[2]!=header[2] || data[3]!=header[3]) { ++current->bad; return false; }
...
void runI2c(unsigned slot, uint32_t clockHz, uint8_t sensor) {
  current = &results[slot];
  current->name = sensor == 1 ? "I2C_100K_ACCEL" : sensor == 2 ? "I2C_100K_GYRO" : sensor == 3 ? "I2C_100K_MAG" : "I2C_100K_RAW_GYRO";
...
void printSummary() {
  ...
  for (unsigned i=0;i<5;++i) {
    ...
    Serial.printf("SAMPLES%lu SPAN_MS%lu DUP_INPUT%lu LAST_XYZ_RAW %d %d %d ERRORS%u END_MARKERS%u\n",
...
void setup() {
  ...
  delay(500);
  Serial.println("FIRMWARE MAKER_IMU_BUS_DIAGNOSTIC_V2; MOTORS DISABLED; P1 MUST BE GND");
  runI2c(0,100000,1);
  runI2c(1,100000,2);
  runI2c(2,100000,3);
  runI2c(3,100000,0x15);
  runI2c(4,100000,1);
  stopBuses();
