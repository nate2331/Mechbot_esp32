...
// IMU SDA/MISO/TX -> verified SN74LVC245AN 5V-tolerant input -> 3V3 output -> GPIO8.
...
// Disconnect all old host RST/P0/SCL/INT wires. This sketch only receives on GPIO8.
...
void help() {
  Serial.println("IMU_RVC_5V_TEST V1 CUSTOM | UART1 RX8 TX=none | 115200 8N1 | frame=19 | RX buffer=8192");
  Serial.println("Only the buffer's 3V3 output connects to GPIO8. IMU TX must not connect directly to S3.");
  Serial.println("Keep IMU 5V OFF. Send r; after ARMED, apply IMU 5V. GPIO8 must idle HIGH for 2ms before UART starts.");
...
void finish(const char* reason) {
  ...
  // Stop the UART event task before freezing its counters. Return only RX8 to input.
...
void armTrial() {
  ...
  Serial.printf("ARMED #%lu RX8 input. APPLY IMU 5V NOW; waiting for idle HIGH >=2ms, then first byte (120s total).\n", (unsigned long)trial);
}
