...
These notes describe the original shared gyro-example HAL port. This
diagnostic reuses those HAL files unchanged; its harness selects 100 kHz
(400 kHz for `h`), performs polling without an ISR, and does not drive an LED.
See `README.md` for the diagnostic's separate behavior.
