...
class MakerBnoSpi : public Adafruit_BNO08x {
 public:
  static constexpr unsigned TRACE_CAPACITY = 48;
  static constexpr unsigned TRACE_PREFIX_SIZE = 32;
  struct TraceEntry {
    uint32_t timestampMs = 0;
    uint16_t length = 0;
    char direction = '?';
    uint8_t prefixLength = 0;
    uint8_t prefix[TRACE_PREFIX_SIZE] = {};
  };

  static constexpr unsigned ERROR_CAPACITY = 32;
  struct ErrorRecord {
    uint8_t severity = 0;
    uint8_t sequence = 0;
    uint8_t source = 0;
  ...
  bool begin_SPI(uint8_t csPin, uint8_t intPin, SPIClass* spi = &SPI,
    ...
    traceCount_ = 0;
    traceOverflowCount_ = 0;
    nextControlSequence_ = 0;
    nextErrorCommandSequence_ = 0;
    errorRequestSequence_ = 0;
    resetErrorCollector();
  ...
  unsigned traceCount() const { return traceCount_; }
  const TraceEntry* traceAt(unsigned index) const {
    return index < traceCount_ ? &trace_[index] : nullptr;
  }
  uint32_t traceOverflowCount() const { return traceOverflowCount_; }

  // Diagnostic-only raw queries. These advance the wire sequence, not SH-2's
  // private counter: reinitialize before any later SH-2 control operation.
  bool probeControl(uint8_t reportId, uint8_t argument) {
    if (!((reportId == 0xFE && argument == 0x08) ||
          (reportId == 0xF9 && argument == 0x00))) return false;
    uint8_t packet[] = {6, 0, 2, nextControlSequence_, reportId, argument};
    return writeTransport(&_HAL, packet, sizeof(packet)) == sizeof(packet);
  }

  // One bounded send; the caller services RX with its own overall deadline.
  // Severity0 requests the highest-priority errors (SH-2 manual section6.4.1).
  // Like probeControl, reset before returning to library control operations.
  bool probeErrors() {
    if (!startupReady_ || !spi_) return false;
    resetErrorCollector();
    errorRequestSequence_ = nextErrorCommandSequence_;
    errorRequestActive_ = true;
    uint8_t packet[16] = {16, 0, 2, nextControlSequence_, 0xF2,
                          errorRequestSequence_, 0x01, 0};
    if (writeTransport(&_HAL, packet, sizeof(packet)) != 16) {
      errorRequestActive_ = false;
      return false;
    }
    ++nextErrorCommandSequence_;
    return true;
  }

  unsigned errorCount() const { return errorCount_; }
  const ErrorRecord* errorAt(unsigned index) const {
    return index < errorCount_ ? &errors_[index] : nullptr;
  }
  uint32_t errorOverflowCount() const { return errorOverflowCount_; }
  bool errorResponseComplete() const { return errorResponseComplete_; }
  uint8_t errorRequestSequence() const { return errorRequestSequence_; }
 ...
 private:
  ...
  TraceEntry trace_[TRACE_CAPACITY];
  unsigned traceCount_ = 0;
  uint32_t traceOverflowCount_ = 0;
  uint8_t nextControlSequence_ = 0;
  ErrorRecord errors_[ERROR_CAPACITY];
  unsigned errorCount_ = 0;
  uint32_t errorOverflowCount_ = 0;
  uint8_t nextErrorCommandSequence_ = 0;
  uint8_t errorRequestSequence_ = 0;
  bool errorRequestActive_ = false;
  bool errorResponseComplete_ = false;
  // ESP32 transferBytesNL loads/stores words; even an odd TX length must not
  // make its subsequent RX-tail buffers unaligned.
  alignas(4) uint8_t txScratch_[SH2_HAL_MAX_TRANSFER_IN] = {};
  alignas(4) uint8_t rxScratch_[SH2_HAL_MAX_TRANSFER_IN] = {};
  alignas(4) uint8_t tailRxScratch_[SH2_HAL_MAX_TRANSFER_IN] = {};
  inline static MakerBnoSpi* active_ = nullptr;

  void resetErrorCollector() {
    errorCount_ = 0;
    errorOverflowCount_ = 0;
    errorRequestActive_ = false;
    errorResponseComplete_ = false;
  }

  void collectErrors(const uint8_t* packet, unsigned length) {
    if (!errorRequestActive_ || errorResponseComplete_ || length < 4 ||
        packet[2] != 2 || (packet[1] & 0x80)) return;
    // Control payloads can bundle records beyond the32-byte trace prefix.
    // Never search for F1 inside an unknown/truncated record or continuation.
    for (unsigned cursor = 4; cursor < length;) {
      const uint8_t* report = packet + cursor;
      unsigned reportLength = 0;
      if (report[0] == 0xF1 || report[0] == 0xF8) reportLength = 16;
      else if (report[0] == 0xFC) reportLength = 17;
      else return;
      if (length - cursor < reportLength) return;
      if (report[0] == 0xF1 && report[2] == 0x01 &&
          report[3] == errorRequestSequence_) {
        if (report[7] == 0xFF) {
          errorResponseComplete_ = true;
          return;
        }
        if (errorCount_ < ERROR_CAPACITY) {
          ErrorRecord& error = errors_[errorCount_++];
          error.severity = report[5];
          error.sequence = report[6];
          error.source = report[7];
          error.error = report[8];
          error.module = report[9];
          error.code = report[10];
        } else {
          ++errorOverflowCount_;
        }
      }
  ...
  void recordTrace(char direction, unsigned length, const uint8_t* bytes,
                   unsigned available) {
    if (traceCount_ == TRACE_CAPACITY) {
      ++traceOverflowCount_;
      return;
    }
    TraceEntry& entry = trace_[traceCount_++];
    entry.timestampMs = millis();
    entry.length = static_cast<uint16_t>(length);
    entry.direction = direction;
    entry.prefixLength = static_cast<uint8_t>(
        available < TRACE_PREFIX_SIZE ? available : TRACE_PREFIX_SIZE);
    memcpy(entry.prefix, bytes, entry.prefixLength);
  }
  ...
  // One SHTP exchange owns CS continuously. Send a known TX packet in one SPI
  // call, matching STRDC's write shape; only a longer RX needs a zero-padded
  // tail. Read-only exchanges retain the four-byte header followed by payload.
  void exchange(const uint8_t* tx, unsigned txLen, uint32_t receivedUs) {
    ...
    if (txLen) recordTrace('T', txLen, txScratch_, txLen);
    spi_->beginTransaction(SPISettings(SPI_CLOCK_HZ, MSBFIRST, SPI_MODE3));
    digitalWrite(csPin_, LOW);
    const unsigned firstBurst = txLen ? txLen : 4;
    spi_->transferBytes(txScratch_, rxScratch_, firstBurst);
    ...
    // Invalid RX never drives extra clocks. A valid longer response is completed
    // under the same CS assertion, using aligned buffers even after a21-byte TX.
    if (validRx && rxLen > firstBurst) {
      const unsigned tailLength = rxLen - firstBurst;
      if (txLen) {
        memset(txScratch_, 0, sizeof(txScratch_));
        spi_->transferBytes(txScratch_, tailRxScratch_, tailLength);
        memcpy(rxScratch_ + firstBurst, tailRxScratch_, tailLength);
      } else {
        spi_->transferBytes(txScratch_ + 4, rxScratch_ + 4, tailLength);
      }
    }
    digitalWrite(csPin_, HIGH);
    spi_->endTransaction();
    if (txLen >= 4 && tx[2] == 2 &&
        ((static_cast<unsigned>(tx[0]) |
          (static_cast<unsigned>(tx[1]) << 8)) & 0x7FFFU) == txLen) {
      nextControlSequence_ = static_cast<uint8_t>(tx[3] + 1U);
    }
    if (rxLen) recordTrace('R', rxLen, rxScratch_, validRx ? rxLen : 4);
    if (txLen) ++diagnostics_.txPackets;
    if (validRx) {
      collectErrors(rxScratch_, rxLen);
